;----------------------------------------------------------------------------
;文件名:6-3-2.ASM
;功能:使用CALL指令实现任务切换实验（反映从一个任务的0级程序段切换到另一个任务的3级程序段的状况）
;----------------------------------------------------------------------------
INCLUDE         386SCD.INC     
;----------------------------------------------------------------------------
...
DCODE2			DESC    <0FFFFH,DCODE2SEG,,ATCE,,>		;0级代码段2描述符
DCODE3			DESC    <0FFFFH,DCODE3SEG,,ATCE+DPL3,,>	;3级代码段3描述符(DPL=3)
...
DDATAS          DESC    <0FFFFH,DDATASSEG,,ATDW+DPL3,,>    ;3级源数据段描述符(DPL=3)
DDATAO          DESC    <0FFFFH,3000H,,ATDW+DPL3,,>  	   ;3级目的数据段描述符(DPL=3)

TOTASK3         GATE    <0,TSS3_SEL,,ATTASKGATE,>          ;指向任务0的调用门描述符
;----------------------------------------------------------------------------
...
DCODE2_SEL		=       DCODE2-GDT				;0级代码段2描述符选择子
DCODE3_SEL		=       DCODE3-GDT+RPL3			;3级代码段3描述符选择子
...
;----------------------------------------------------------------------------
TSS1SEG      SEGMENT PARA USE16                ;任务1状态段TSS
TEMPTASK     TSS     <>
             DB      0FFH                      ;I/O许可位图结束标志
TSS1LEN      =       $
TSS1SEG      ENDS								;任务1状态段TSS结束
;----------------------------------------------------------------------------
TSS2SEG      SEGMENT PARA USE16                 ;任务2状态段TSS
                DD      0                       ;链接字
                ...
                DB      0FFH                    ;I/O许可位图结束标志
TSS2LEN      =       $
TSS2SEG      ENDS                              ;任务2状态段TSS结束
;----------------------------------------------------------------------------
TSS3SEG      SEGMENT PARA USE16                ;任务3状态段TSS
                DD      0                       ;链接字
                DD      0        			    ;0级堆栈指针
                DD      0	     		        ;0级堆栈选择子
                DD      0                       ;1级堆栈指针(实例不使用)
                ...
                DB      0FFH                      ;I/O许可位图结束标志
TSS3LEN      =       $
TSS3SEG      ENDS                              ;任务3状态段TSS结束
;----------------------------------------------------------------------------
DSTACK1SEG   	SEGMENT PARA   USE16              ;任务1堆栈段
DSTACK1LEN	=       512
                DB      DSTACK1LEN DUP(?)		  ;定义512个缓冲字节单元
DSTACK1SEG	ENDS                              	  ;任务1堆栈段结束
;----------------------------------------------------------------------------
DSTACK2SEG	SEGMENT PARA	USE16	             ;任务2堆栈段
DSTACK2LEN	=       512
		DB      DSTACK2LEN DUP(?)
DSTACK2SEG	ENDS       
;----------------------------------------------------------------------------
DSTACK3SEG	SEGMENT PARA	USE16	             ;任务3堆栈段
DSTACK3LEN	=       512
		DB      DSTACK3LEN DUP(?)
DSTACK3SEG	ENDS       
;----------------------------------------------------------------------------
...
START		PROC
			...
			MOV     AX,TSS1SEG
			MUL     BX
			MOV     WORD PTR TSSTABLE1.BASEL,AX  ;计算并设置任务段1基地址
			...
			MOV     AX,TSS2SEG
			MUL     BX
			MOV     WORD PTR TSSTABLE2.BASEL,AX  ;计算并设置任务段2基地址
			...
			MOV     AX,TSS3SEG
			MUL     BX
			MOV     WORD PTR TSSTABLE3.BASEL,AX  ;计算并设置任务段3基地址
...
VIRTUAL:    ;现在开始在保护方式下运行
			...
			LTR		AX
			MOV		AX,DDATA1_SEL				;置数据段1
			MOV		DS,AX
			MOV		ES,AX
			MOV		FS,AX
			MOV		GS,AX
			MOV		ESP,DSTACK1LEN-1
			MOV		AX,DSTACK1_SEL				;置堆栈段1
			MOV		SS,AX
...
DCODE2SEG	SEGMENT PARA USE16               	 ;任务2的0级代码段
              ASSUME  CS:DCODE2SEG	
                ...
                CALL16  TOTASK3_SEL,0			;调用任务门描述符到任务3
